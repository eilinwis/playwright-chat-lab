...
async function sendMessageInChat(page: Page, text: string, expectedReplyCount: number) {
...
test.describe('Homework 3: Assertions & auto-waiting', () => {
  test('history starts empty, then fills up and enables delete as messages are sent', async ({ page }) => {
    ...
    await page.goto('/')
    await sendMessageInChat(page, 'Gravity always wins', 1)
    await sendMessageInChat(page, 'How odd', 2)
    await sendMessageInChat(page, 'If only', 3)
