...
test.describe('Homework 5: Custom widgets & complex interactions', () => {
  ...
  test('combining a category toggle with a search query narrows and resets the results', async ({ page }) => {
    ...
    await page.getByTestId('filter-category-frontend').click()
    await expect(page.getByTestId('filter-category-frontend')).toHaveAttribute('aria-pressed', 'true')
    ...
    await page.getByTestId('filter-category-frontend').click()
    await expect(page.getByTestId('filter-category-frontend')).toHaveAttribute('aria-pressed', 'false')
